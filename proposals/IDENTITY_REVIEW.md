...
- Guard tightened this round: equal-title match, multilingual aliases (Creation/Schöpfung, Matthew/Matthäus), `symphonien` form family, key conflicts (C minor ≠ B minor), weak tokens (`don`/`st`) so Don Giovanni ↛ Don Carlo.

## Round status (Role B · densified seed)

| Metric | Count |
|---|---:|
| Seed candidates | 511 |
| Identity proposals this file | 241 |
| `auto_accept_eligible` (≥80, no flags) | 71 |
| Flagged / uncertain | 170 |
| `wrong work:` (never apply) | 21 |
| **Remaining without identity proposal** | **270** |

Budget **300** requests · contact `harvest@example.invalid` (placeholder — note for MusicBrainz; set a real `HARVEST_CONTACT` next round). ~86 HTTP 503s burned budget without advancing those cursors — retry next round.

**Stop for human identity review. Do not apply.** Next harvest round should resume from `.cache/harvest_state.json` (local) toward the remaining 270 candidates.
