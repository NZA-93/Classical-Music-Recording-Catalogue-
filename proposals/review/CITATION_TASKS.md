# Citation tasks (queued)

These are **tasks**, not statements. A human (or Role C with a locator) turns each into a ratified contribution — agents still must not write `data/statements/`.

**Count:** 510

| Target | Source hint | Note |
|---|---|---|
| `bach/brandenburg/3` | — |  |
| `bach/violin_concertos/3` | — |  |
| `bach/sonatas_partitas/0` | — |  |
| `bach/goldberg/2` | — |  |
| `bach/wtc/0` | — |  |
| `bach/mass_b_minor/1` | — |  |
| `bach/mass_b_minor/2` | — |  |
| `bach/matthew/0` | — |  |
| `bach/matthew/2` | — |  |
| `bach/matthew/3` | — |  |
| `bach/john/0` | — |  |
| `bach/john/2` | — |  |
| `bach/art_of_fugue/0` | — |  |
| `bach/art_of_fugue/4` | — |  |
| `beethoven/sym3/0` | — |  |
| `beethoven/sym3/2` | — |  |
| `beethoven/sym5/0` | — |  |
| `beethoven/sym5/2` | — |  |
| `beethoven/sym6/4` | — |  |
| `beethoven/sym9/0` | — |  |
| `beethoven/sym9/1` | — |  |
| `beethoven/pc5/0` | — |  |
| `beethoven/pc5/3` | — |  |
| `beethoven/violin_concerto/0` | — |  |
| `beethoven/missa_solemnis/2` | — |  |
| `mozart/pc20/0` | — |  |
| `mozart/requiem/0` | — |  |
| `mozart/figaro/1` | — |  |
| `mozart/don_giovanni/0` | — |  |
| `mozart/don_giovanni/1` | — |  |
| `mozart/cosi/0` | — |  |
| `mozart/zauberflote/0` | — |  |
| `mozart/zauberflote/1` | — |  |
| `puccini/boheme/0` | — |  |
| `puccini/boheme/3` | — |  |
| `puccini/tosca/1` | — |  |
| `puccini/tosca/3` | — |  |
| `puccini/butterfly/1` | — |  |
| `puccini/fanciulla/0` | — |  |
| `puccini/turandot/1` | — |  |
| `shostakovich/sym5/0` | — |  |
| `shostakovich/sym5/1` | — |  |
| `shostakovich/sym5/3` | — |  |
| `shostakovich/sym10/0` | — |  |
| `shostakovich/sym13/0` | — |  |
| `brahms/sym1/0` | — |  |
| `brahms/sym4/1` | — |  |
| `brahms/pc1/1` | — |  |
| `brahms/pc2/1` | — |  |
| `brahms/violin_concerto/1` | — |  |
| `brahms/clarinet_quintet/0` | — |  |
| `brahms/piano_quintet/0` | — |  |
| `brahms/liebeslieder/0` | — |  |
| `haydn/cello_concerto1/0` | — |  |
| `haydn/seasons/0` | — |  |
| `haydn/nelson_mass/0` | — |  |
| `schubert/sym9/0` | — |  |
| `schubert/winterreise/0` | — |  |
| `schubert/winterreise/1` | — |  |
| `schubert/schone_mullerin/0` | — |  |
| `schubert/string_quintet/0` | — |  |
| `schubert/trout/0` | — |  |
| `schubert/schwanengesang/1` | — |  |
| `handel/messiah/1` | — |  |
| `handel/alcina/0` | — |  |
| `chopin/sonata2/0` | — |  |
| `chopin/pc1/1` | — |  |
| `chopin/pc2/0` | — |  |
| `bach/brandenburg/0` | — |  |
| `bach/brandenburg/2` | — |  |
| `bach/brandenburg/4` | — |  |
| `bach/brandenburg/5` | — |  |
| `bach/suites/0` | — |  |
| `bach/suites/1` | — |  |
| `bach/suites/3` | — |  |
| `bach/violin_concertos/0` | — |  |
| `bach/violin_concertos/2` | — |  |
| `bach/cello_suites/0` | — |  |
| `bach/cello_suites/3` | — |  |
| `bach/sonatas_partitas/1` | — |  |
| `bach/sonatas_partitas/3` | — |  |
| `bach/goldberg/1` | — |  |
| `bach/goldberg/3` | — |  |
| `bach/goldberg/4` | — |  |
| `bach/wtc/2` | — |  |
| `bach/wtc/3` | — |  |
| `bach/wtc/4` | — |  |
| `bach/mass_b_minor/0` | — |  |
| `bach/matthew/1` | — |  |
| `bach/john/4` | — |  |
| `bach/art_of_fugue/2` | — |  |
| `bach/harpsichord_concertos/0` | — |  |
| `bach/harpsichord_concertos/2` | — |  |
| `bach/harpsichord_concertos/4` | — |  |
| `beethoven/sym3/1` | — |  |
| `beethoven/sym6/0` | — |  |
| `beethoven/sym6/1` | — |  |
| `beethoven/sym6/2` | — |  |
| `beethoven/sym6/3` | — |  |
| `beethoven/sym7/0` | — |  |
| `beethoven/sym7/1` | — |  |
| `beethoven/sym7/3` | — |  |
| `beethoven/sym9/3` | — |  |
| `beethoven/pc4/0` | — |  |
| `beethoven/pc4/2` | — |  |
| `beethoven/pc4/4` | — |  |
| `beethoven/pc5/2` | — |  |
| `beethoven/violin_concerto/2` | — |  |
| `beethoven/violin_concerto/3` | — |  |
| `beethoven/late_quartets/0` | — |  |
| `beethoven/late_quartets/1` | — |  |
| `beethoven/late_quartets/2` | — |  |
| `beethoven/late_quartets/3` | — |  |
| `beethoven/hammerklavier/0` | — |  |
| `beethoven/hammerklavier/1` | — |  |
| `beethoven/hammerklavier/3` | — |  |
| `beethoven/missa_solemnis/1` | — |  |
| `beethoven/fidelio/1` | — |  |
| `beethoven/fidelio/3` | — |  |
| `mozart/late_symphonies/1` | — |  |
| `mozart/late_symphonies/3` | — |  |
| `mozart/pc20/2` | — |  |
| `mozart/pc20/3` | — |  |
| `mozart/pc23/0` | — |  |
| `mozart/pc23/1` | — |  |
| `mozart/pc23/2` | — |  |
| `mozart/pc23/3` | — |  |
| `mozart/pc23/4` | — |  |
| `mozart/pc27/0` | — |  |
| `mozart/pc27/2` | — |  |
| `mozart/pc27/4` | — |  |
| `mozart/clarinet_concerto/2` | — |  |
| `mozart/requiem/1` | — |  |
| `mozart/requiem/3` | — |  |
| `mozart/figaro/3` | — |  |
| `mozart/don_giovanni/2` | — |  |
| `mozart/don_giovanni/3` | — |  |
| `mozart/cosi/1` | — |  |
| `mozart/cosi/2` | — |  |
| `mozart/cosi/3` | — |  |
| `mozart/zauberflote/2` | — |  |
| `mozart/zauberflote/3` | — |  |
| `mozart/mass_c_minor/1` | — |  |
| `mozart/mass_c_minor/3` | — |  |
| `mozart/string_quintets/0` | — |  |
| `mozart/string_quintets/2` | — |  |
| `mozart/string_quintets/3` | — |  |
| `mozart/string_quintets/4` | — |  |
| `puccini/manon_lescaut/1` | — |  |
| `puccini/manon_lescaut/2` | — |  |
| `puccini/manon_lescaut/3` | — |  |
| `puccini/boheme/2` | — |  |
| `puccini/tosca/4` | — |  |
| `puccini/butterfly/3` | — |  |
| `puccini/fanciulla/1` | — |  |
| `puccini/fanciulla/3` | — |  |
| `puccini/rondine/1` | — |  |
| `puccini/rondine/3` | — |  |
| `puccini/trittico/0` | — |  |
| `puccini/trittico/2` | — |  |
| `puccini/trittico/4` | — |  |
| `puccini/turandot/3` | — |  |
| `puccini/villi/0` | — |  |
| `puccini/villi/1` | — |  |
| `puccini/villi/2` | — |  |
| `puccini/villi/3` | — |  |
| `puccini/villi/4` | — |  |
| `puccini/edgar/0` | — |  |
| `puccini/edgar/2` | — |  |
| `puccini/edgar/4` | — |  |
| `puccini/messa_gloria/1` | — |  |
| `puccini/messa_gloria/3` | — |  |
| `shostakovich/sym1/0` | — |  |
| `shostakovich/sym1/2` | — |  |
| `shostakovich/sym1/3` | — |  |
| `shostakovich/sym1/4` | — |  |
| `shostakovich/sym4/0` | — |  |
| `shostakovich/sym4/1` | — |  |
| `shostakovich/sym4/2` | — |  |
| `shostakovich/sym4/3` | — |  |
| `shostakovich/sym4/4` | — |  |
| `shostakovich/sym5/2` | — |  |
| `shostakovich/sym5/4` | — |  |
| `shostakovich/sym5/6` | — |  |
| `shostakovich/sym6/1` | — |  |
| `shostakovich/sym6/3` | — |  |
| `shostakovich/sym7/0` | — |  |
| `shostakovich/sym7/2` | — |  |
| `shostakovich/sym8/0` | — |  |
| `shostakovich/sym8/1` | — |  |
| `shostakovich/sym8/2` | — |  |
| `shostakovich/sym8/3` | — |  |
| `shostakovich/sym9/0` | — |  |
| `shostakovich/sym9/1` | — |  |
| `shostakovich/sym9/3` | — |  |
| `shostakovich/sym10/1` | — |  |
| `shostakovich/sym10/3` | — |  |
| `shostakovich/sym11/0` | — |  |
| `shostakovich/sym11/2` | — |  |
| `shostakovich/sym11/3` | — |  |
| `shostakovich/sym11/4` | — |  |
| `shostakovich/sym13/1` | — |  |
| `shostakovich/sym13/2` | — |  |
| `shostakovich/sym13/3` | — |  |
| `shostakovich/sym14/0` | — |  |
| `shostakovich/sym14/1` | — |  |
| `bach/brandenburg/1` | — |  |
| `bach/cello_suites/1` | — |  |
| `bach/wtc/1` | — |  |
| `bach/john/3` | — |  |
| `beethoven/sym3/3` | — |  |
| `beethoven/sym5/1` | — |  |
| `beethoven/sym5/3` | — |  |
| `beethoven/missa_solemnis/0` | — |  |
| `mozart/clarinet_concerto/0` | — |  |
| `mozart/figaro/0` | — |  |
| `puccini/manon_lescaut/0` | — |  |
| `puccini/boheme/1` | — |  |
| `puccini/tosca/0` | — |  |
| `puccini/tosca/2` | — |  |
| `puccini/butterfly/0` | — |  |
| `puccini/butterfly/2` | — |  |
| `puccini/turandot/0` | — |  |
| `puccini/turandot/2` | — |  |
| `brahms/sym1/1` | — |  |
| `brahms/pc1/0` | — |  |
| `brahms/pc2/0` | — |  |
| `brahms/violin_concerto/0` | — |  |
| `brahms/german_requiem/0` | — |  |
| `brahms/clarinet_quintet/1` | — |  |
| `brahms/haydn_variations/0` | — |  |
| `haydn/creation/0` | — |  |
| `schubert/death_maiden/1` | — |  |
| `schubert/schwanengesang/0` | — |  |
| `handel/messiah/0` | — |  |
| `handel/fireworks/1` | — |  |
| `chopin/etudes/0` | — |  |
| `chopin/sonata3/1` | — |  |
| `bach/suites/2` | — |  |
| `bach/suites/4` | — |  |
| `bach/violin_concertos/1` | — |  |
| `bach/violin_concertos/4` | — |  |
| `bach/cello_suites/2` | — |  |
| `bach/cello_suites/4` | — |  |
| `bach/sonatas_partitas/2` | — |  |
| `bach/mass_b_minor/3` | — |  |
| `bach/john/1` | — |  |
| `bach/art_of_fugue/1` | — |  |
| `bach/art_of_fugue/3` | — |  |
| `bach/harpsichord_concertos/1` | — |  |
| `bach/harpsichord_concertos/3` | — |  |
| `beethoven/sym7/2` | — |  |
| `beethoven/sym9/2` | — |  |
| `beethoven/sym9/4` | — |  |
| `beethoven/pc4/1` | — |  |
| `beethoven/pc4/3` | — |  |
| `beethoven/pc5/1` | — |  |
| `beethoven/violin_concerto/1` | — |  |
| `beethoven/hammerklavier/2` | — |  |
| `beethoven/hammerklavier/4` | — |  |
| `beethoven/missa_solemnis/3` | — |  |
| `beethoven/fidelio/2` | — |  |
| `mozart/late_symphonies/0` | — |  |
| `mozart/late_symphonies/2` | — |  |
| `mozart/pc20/1` | — |  |
| `mozart/pc27/1` | — |  |
| `mozart/pc27/3` | — |  |
| `mozart/clarinet_concerto/1` | — |  |
| `mozart/clarinet_concerto/3` | — |  |
| `mozart/requiem/2` | — |  |
| `mozart/figaro/2` | — |  |
| `mozart/mass_c_minor/0` | — |  |
| `mozart/mass_c_minor/2` | — |  |
| `mozart/mass_c_minor/4` | — |  |
| `mozart/string_quintets/1` | — |  |
| `puccini/fanciulla/2` | — |  |
| `puccini/rondine/0` | — |  |
| `puccini/rondine/2` | — |  |
| `puccini/rondine/4` | — |  |
| `puccini/trittico/1` | — |  |
| `puccini/edgar/1` | — |  |
| `puccini/edgar/3` | — |  |
| `puccini/messa_gloria/0` | — |  |
| `puccini/messa_gloria/2` | — |  |
| `puccini/messa_gloria/4` | — |  |
| `shostakovich/sym1/1` | — |  |
| `shostakovich/sym5/5` | — |  |
| `shostakovich/sym6/0` | — |  |
| `shostakovich/sym6/2` | — |  |
| `shostakovich/sym6/4` | — |  |
| `shostakovich/sym7/1` | — |  |
| `shostakovich/sym7/3` | — |  |
| `shostakovich/sym9/2` | — |  |
| `shostakovich/sym9/4` | — |  |
| `shostakovich/sym10/2` | — |  |
| `shostakovich/sym10/4` | — |  |
| `shostakovich/sym14/2` | — |  |
| `shostakovich/sym14/4` | — |  |
| `shostakovich/sym15/1` | — |  |
| `shostakovich/sym15/2` | — |  |
| `shostakovich/sym15/3` | — |  |
| `shostakovich/sym15/4` | — |  |
| `brahms/sym1/2` | — |  |
| `brahms/sym1/3` | — |  |
| `brahms/sym1/4` | — |  |
| `brahms/sym4/0` | — |  |
| `brahms/sym4/2` | — |  |
| `brahms/sym4/3` | — |  |
| `brahms/pc1/2` | — |  |
| `brahms/pc1/3` | — |  |
| `brahms/pc2/3` | — |  |
| `brahms/violin_concerto/3` | — |  |
| `brahms/german_requiem/2` | — |  |
| `brahms/clarinet_quintet/2` | — |  |
| `brahms/clarinet_quintet/3` | — |  |
| `brahms/piano_quintet/1` | — |  |
| `brahms/piano_quintet/3` | — |  |
| `brahms/haydn_variations/2` | — |  |
| `brahms/haydn_variations/3` | — |  |
| `brahms/liebeslieder/1` | — |  |
| `brahms/liebeslieder/2` | — |  |
| `brahms/academic_festival/0` | — |  |
| `brahms/academic_festival/2` | — |  |
| `brahms/tragic_overture/0` | — |  |
| `brahms/tragic_overture/2` | — |  |
| `haydn/sym104/0` | — |  |
| `haydn/sym104/1` | — |  |
| `haydn/sym104/2` | — |  |
| `haydn/sym104/3` | — |  |
| `haydn/sym94/0` | — |  |
| `haydn/sym94/1` | — |  |
| `haydn/sym94/2` | — |  |
| `haydn/sym94/3` | — |  |
| `haydn/creation/1` | — |  |
| `haydn/creation/2` | — |  |
| `haydn/creation/3` | — |  |
| `haydn/op76/0` | — |  |
| `haydn/op76/2` | — |  |
| `haydn/op76/3` | — |  |
| `haydn/trumpet_concerto/0` | — |  |
| `haydn/trumpet_concerto/1` | — |  |
| `haydn/trumpet_concerto/2` | — |  |
| `haydn/trumpet_concerto/3` | — |  |
| `haydn/cello_concerto1/1` | — |  |
| `haydn/cello_concerto1/2` | — |  |
| `haydn/seasons/1` | — |  |
| `haydn/seasons/3` | — |  |
| `haydn/nelson_mass/2` | — |  |
| `haydn/piano_sonatas/0` | — |  |
| `haydn/piano_sonatas/2` | — |  |
| `haydn/piano_sonatas/3` | — |  |
| `haydn/sym44/0` | — |  |
| `haydn/sym44/1` | — |  |
| `haydn/sym44/2` | — |  |
| `haydn/sym44/3` | — |  |
| `haydn/sym45/0` | — |  |
| `haydn/sym45/1` | — |  |
| `haydn/sym45/2` | — |  |
| `haydn/seven_last_words/0` | — |  |
| `haydn/seven_last_words/1` | — |  |
| `haydn/seven_last_words/2` | — |  |
| `schubert/sym8/0` | — |  |
| `schubert/sym8/2` | — |  |
| `schubert/sym9/1` | — |  |
| `schubert/sym9/3` | — |  |
| `schubert/winterreise/2` | — |  |
| `schubert/winterreise/3` | — |  |
| `schubert/schone_mullerin/1` | — |  |
| `schubert/schone_mullerin/2` | — |  |
| `schubert/schone_mullerin/3` | — |  |
| `schubert/string_quintet/1` | — |  |
| `schubert/string_quintet/2` | — |  |
| `schubert/string_quintet/3` | — |  |
| `schubert/death_maiden/0` | — |  |
| `schubert/death_maiden/2` | — |  |
| `schubert/sonata_d960/0` | — |  |
| `schubert/sonata_d960/2` | — |  |
| `schubert/sonata_d960/4` | — |  |
| `schubert/trout/1` | — |  |
| `schubert/trout/2` | — |  |
| `schubert/trout/3` | — |  |
| `schubert/mass_e_flat/0` | — |  |
| `schubert/mass_e_flat/1` | — |  |
| `schubert/mass_e_flat/2` | — |  |
| `schubert/mass_e_flat/3` | — |  |
| `schubert/impromptus/0` | — |  |
| `schubert/impromptus/1` | — |  |
| `schubert/impromptus/3` | — |  |
| `schubert/schwanengesang/3` | — |  |
| `schubert/rosamunde/1` | — |  |
| `schubert/rosamunde/3` | — |  |
| `handel/messiah/2` | — |  |
| `handel/messiah/3` | — |  |
| `handel/water_music/0` | — |  |
| `handel/water_music/1` | — |  |
| `handel/water_music/2` | — |  |
| `handel/water_music/3` | — |  |
| `handel/fireworks/0` | — |  |
| `handel/fireworks/2` | — |  |
| `handel/fireworks/3` | — |  |
| `handel/giulio_cesare/0` | — |  |
| `handel/giulio_cesare/1` | — |  |
| `handel/giulio_cesare/2` | — |  |
| `handel/giulio_cesare/3` | — |  |
| `handel/alcina/1` | — |  |
| `handel/alcina/2` | — |  |
| `handel/op6/0` | — |  |
| `handel/op6/1` | — |  |
| `handel/op6/2` | — |  |
| `handel/op6/3` | — |  |
| `handel/organ_concertos/0` | — |  |
| `handel/organ_concertos/1` | — |  |
| `handel/organ_concertos/2` | — |  |
| `handel/organ_concertos/3` | — |  |
| `handel/israel_in_egypt/1` | — |  |
| `handel/israel_in_egypt/3` | — |  |
| `handel/solomon/1` | — |  |
| `handel/solomon/3` | — |  |
| `handel/agrippina/1` | — |  |
| `handel/rodelinda/0` | — |  |
| `handel/rodelinda/1` | — |  |
| `handel/rodelinda/2` | — |  |
| `handel/rodelinda/3` | — |  |
| `handel/dixit_dominus/0` | — |  |
| `handel/dixit_dominus/1` | — |  |
| `handel/dixit_dominus/2` | — |  |
| `handel/dixit_dominus/3` | — |  |
| `chopin/ballades/0` | — |  |
| `chopin/ballades/1` | — |  |
| `chopin/ballades/3` | — |  |
| `chopin/etudes/2` | — |  |
| `chopin/preludes/0` | — |  |
| `chopin/preludes/2` | — |  |
| `chopin/sonata2/1` | — |  |
| `chopin/sonata2/3` | — |  |
| `chopin/sonata3/0` | — |  |
| `chopin/sonata3/2` | — |  |
| `chopin/sonata3/3` | — |  |
| `chopin/pc1/0` | — |  |
| `chopin/pc1/2` | — |  |
| `chopin/pc1/3` | — |  |
| `chopin/pc2/1` | — |  |
| `chopin/pc2/2` | — |  |
| `chopin/pc2/3` | — |  |
| `chopin/nocturnes/1` | — |  |
| `chopin/nocturnes/3` | — |  |
| `chopin/polonaises/1` | — |  |
| `chopin/polonaises/3` | — |  |
| `chopin/mazurkas/1` | — |  |
| `chopin/mazurkas/3` | — |  |
| `chopin/scherzi/0` | — |  |
| `chopin/scherzi/1` | — |  |
| `chopin/scherzi/2` | — |  |
| `bach/goldberg/0` | — |  |
| `beethoven/fidelio/0` | — |  |
| `puccini/trittico/3` | — |  |
| `shostakovich/sym11/1` | — |  |
| `shostakovich/sym14/3` | — |  |
| `shostakovich/sym15/0` | — |  |
| `brahms/pc2/2` | — |  |
| `brahms/violin_concerto/2` | — |  |
| `brahms/german_requiem/1` | — |  |
| `brahms/german_requiem/3` | — |  |
| `brahms/piano_quintet/2` | — |  |
| `brahms/haydn_variations/1` | — |  |
| `brahms/liebeslieder/3` | — |  |
| `brahms/academic_festival/1` | — |  |
| `brahms/academic_festival/3` | — |  |
| `brahms/tragic_overture/1` | — |  |
| `brahms/tragic_overture/3` | — |  |
| `haydn/op76/1` | — |  |
| `haydn/cello_concerto1/3` | — |  |
| `haydn/seasons/2` | — |  |
| `haydn/nelson_mass/1` | — |  |
| `haydn/nelson_mass/3` | — |  |
| `haydn/piano_sonatas/1` | — |  |
| `haydn/seven_last_words/3` | — |  |
| `schubert/sym8/1` | — |  |
| `schubert/sym8/3` | — |  |
| `schubert/sym9/2` | — |  |
| `schubert/death_maiden/3` | — |  |
| `schubert/sonata_d960/1` | — |  |
| `schubert/impromptus/2` | — |  |
| `schubert/schwanengesang/2` | — |  |
| `schubert/rosamunde/0` | — |  |
| `schubert/rosamunde/2` | — |  |
| `handel/alcina/3` | — |  |
| `handel/israel_in_egypt/0` | — |  |
| `handel/israel_in_egypt/2` | — |  |
| `handel/solomon/0` | — |  |
| `handel/solomon/2` | — |  |
| `handel/agrippina/0` | — |  |
| `handel/agrippina/2` | — |  |
| `chopin/ballades/2` | — |  |
| `chopin/etudes/1` | — |  |
| `chopin/etudes/3` | — |  |
| `chopin/preludes/1` | — |  |
| `chopin/preludes/3` | — |  |
| `chopin/sonata2/2` | — |  |
| `chopin/nocturnes/0` | — |  |
| `chopin/nocturnes/2` | — |  |
| `chopin/polonaises/0` | — |  |
| `chopin/polonaises/2` | — |  |
| `chopin/mazurkas/0` | — |  |
| `chopin/mazurkas/2` | — |  |
| `chopin/scherzi/3` | — |  |
| `chopin/impromptus/0` | — |  |
| `chopin/impromptus/1` | — |  |
| `chopin/impromptus/2` | — |  |
| `chopin/impromptus/3` | — |  |
