...
# Map award / first-choice names onto Statement class keys so weight() encodes
# review origin (Cls × Prov × conflict). Prefer public award facts.
_MAJOR_AWARD = re.compile(
    r"grammy|gramophone award|diapason d['’]?or|choc de classica|"
    r"preis der deutschen schallplattenkritik|echo klassik|bbc music magazine award",
    re.I,
)
_SPECIALIST_SURVEY = re.compile(
    r"editor'?s choice|building a library|penguin|guide to the|"
    r"record guide|indispensable",
    re.I,
)


def source_class(award_name: str) -> str:
    """Return the proposal `class` key reflecting the award's origin."""
    name = award_name or ""
    if _MAJOR_AWARD.search(name):
        return "major_award"
    if _SPECIALIST_SURVEY.search(name):
        return "specialist survey"
    # Default conservatively: unknown structured lists stay major_award only
    # when they look like named prizes; otherwise specialist survey.
    if re.search(r"\b(award|prize|prix|premio)\b", name, re.I):
        return "major_award"
    return "specialist survey"
...
def proposal_for(award: dict, hit: dict) -> dict:
    ...
    if len(char) > MAX_CHARACTERISATION:
        char = char[: MAX_CHARACTERISATION - 1].rstrip() + "…"
    caveat = award.get("locator_caveat")
    note = award.get("note") or ""
    if caveat:
        note = (note + " " if note else "") + caveat
    cls = source_class(award.get("award") or "")
    return {
        "target": rec["id"],
        "kind": "award",
        "payload": {
...
def run(dry_run: bool = False, quiet: bool = False) -> list[dict]:
    ...
    for award in awards:
        if not award.get("locator"):
            log(f"  skip (no locator): {award.get('award')} {award.get('year')}")
            continue
        hits = match_award(award, recordings)
        if not hits:
            log(f"  no match: {award.get('award')} {award.get('year')} "
                f"({award.get('performers')})")
            continue
        # One proposal per (award, recording). Album awards share covers_works.
        for hit in hits:
            prop = proposal_for(award, hit)
            proposals.append(prop)
            flag = " AMBIGUOUS" if hit["ambiguous_risk"] else ""
            log(f"  match{flag}: {prop['target']} ← {award.get('award')} "
                f"{award.get('year')}")
    return proposals
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    proposals = run(dry_run=args.dry_run, quiet=args.quiet)
